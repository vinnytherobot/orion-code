...
export class RefreshTokenRepository {
  ...
  async findByToken(token: string): Promise<RefreshToken | null> {
    const result = await this.db.select().from(schema.refreshTokens).where(eq(schema.refreshTokens.token, token)).limit(1);
  ...
  async delete(token: string): Promise<boolean> {
    const result = await this.db.delete(schema.refreshTokens).where(eq(schema.refreshTokens.token, token)).returning();
    return result.length > 0;
  }

  async deleteByUserId(userId: string): Promise<number> {
    const result = await this.db.delete(schema.refreshTokens).where(eq(schema.refreshTokens.userId, userId)).returning();
